...
#' Binomial (MSB) model. It builds a *model frame* for a MSB model specified
...
#' component.
#'
#' @param formula an object of class "\link[stats]{formula}" (or one that can
#' be coerced to that class), a symbolic description of the MSB model.
#' The details of model specification are given under section **Details**.
#'
#' @param input.me optional named list of one sided \link{formula} objects
...
#' See section **Details** for specification rules.
#'
#' @param stage.me optional one sided \link{formula} object specifying
...
#' See section **Details**.
#'
# @param me.model optional named list of one sided \link{formula} objects
# specifying predictors for the standard deviations of measurement errors in
# explanatory variables included in \code{formula}.
# The default assumes no estimable measurement error in the MSB model
# (this is currently the only available option).
#
# This is a planned feature intended to allow flexibility in the MSB model.
# See **Details**.
#'
#' @param alpha.formula optional one sided \link{formula} object specifying
...
#' is zero.
#'
#' @param lambda.formula optional one sided \link{formula} object specifying
...
#'
#' @param kappa.formula optional one sided \link{formula} object specifying
...
#' when \eqn{\kappa = 1}.
#'
#' @param data a data frame (or object coercible by \link{as.data.frame} to a
#' data frame), list or environment, containing the variables in \code{formula}.
#' Neither a matrix nor an array is accepted.
#'
#' @param weights an optional vector of *number of trials* ran to obtain the
#' response specified in \code{formula}. The \code{weights} indicate the
#' sampling effort corresponding to each data point (row).
#'
#' @param sample.weights an optional vector of non-negative real,
#' indicating how representative each data point is vis-a-vis the
#' target population (see section *Observation weights* in **Details**).
#' Should be \code{NULL} or a \code{numeric} vector.
#'
#' @param subset a specification of the rows/observations to be used: defaults
#' to all (see \link[stats]{model.frame} for details).
#'
#' @param na.action an optional (name of a) function for treating missing values
...
#'
#' @param frames logical, should the \link[stats]{model.frame}s corresponding to
#' different model components be returned? Defaults to \code{FALSE}.
#'
#' @param verbose integer, should information be printed while building the
#' model frame? A positive value means yes, and larger value might provide
#' more details.
#'
#' @param offset not used, included to prevent the inclusion of an \link{offset}
#' into the MSB model specification through \code{...}.
#' See section *Including offsets* in **Details** for the right way to specify
#' offsets in the MSB model.
#'
#' @param ... extra arguments for future methods (to pass to or from).
#' Currently only passed to \link[stats]{model.frame}.
#'
#' @details
#' **Processing**
#'
#' The function builds a set of design matrices (see section **Value**)
...
#'
# Note that if all the values in the offset column specified for a stage are
# equal, only a scalar is returned in the model frame as \code{offset} for
# the stage.
#'
#' **Observation weights**
#'
...
#'
#' *PS*: Only stages, each with a non-null offset or at least
...
#' When the stage missing standard deviation offset *is not* in
...
#' \code{stage.me = ~ 0 | Eroff2 |}.
#'
#' *PS*: For this particular example, we can rearrange the argument
...
#'
# The argument \code{me.model} is not currently used.
# It is related to a planned feature for the MSB model implementation in this
# package: predictors specified in \code{me.model} will define measurement
# error models for some numerical covariates in any stage.
# Indeed, we may only know that the variability of a
# covariate is determined by some predictors. The argument \code{me.model}
# allows the user to possibly supply such predictors for each covariate in
# \link{formula}. Then, a link model (e.g. \code{"log"} to ensure positivity,
# or \code{"logit"} for predictors restricted to \code{[0, 1]}) is assigned
# to each standard deviation with a multiplicative offset taken from formulae
# in \code{input.me} (offsets are set to one if \code{input.me} is an empty
# list, the default). This feature is under experimentation and may be
# available in future releases.
#
#' @return an object of class \code{"msbm.frame"} which is a
#' \link[base]{list} with at least the following elements:
#'
#' \item{\code{y}}{ the binomial response vector;}
#'
#' \item{\code{weights}}{ the *number of trials* for each binomial response;
...
#'
#' \item{\code{sample.weights}}{ the importance of the *population patch*
...
#' the *j*th element gives, for the *j*th stage, the vector of column numbers of
...
#' @export msbm.frame
#'
#' @importFrom stats model.frame is.empty.model model.matrix model.response
#' @importFrom stats model.offset model.weights as.formula na.pass na.omit
# @importFrom stringi stri_split_fixed
#'
#' @seealso See \link[msbreg]{simulate.msbm.frame} for \link[stats]{simulate}
...
#' mframe
#'
# Currently not used (planned argument, not yet implemented)
# me.model: specify a ME model (e.g. log-linear) with covariates and coefficients to be estimated
msbm.frame <- function (formula,
                        input.me = list(),
                        stage.me = NULL, #  me.model = list(),
                        alpha.formula = ~ 0,
                        lambda.formula = ~ 1,
                        kappa.formula = ~ 0,
                        ...
                        drop.unused.levels = FALSE, offset,
                        frames = FALSE, verbose = TRUE, ...) {
  # Matched call
  mcall <- match.call()

  # Routine to display warnings
  eval(toget_verbosewarning.frame())

  # Save options
  Glob.na.action <- options('na.action')$na.action
  options(na.action = 'na.pass')

  # Build main model frame
  eval(toget_main.frame())

  # Binary tables and term orders
  eval(toget_main.binary.tables())

  # Stage model matrix and Binary table of contrasts
  eval(toget_main.stage.matrix())

  # alpha model, lambda and kappa component
  eval(toget_alphalambda.comps())

  # sample.weights
  eval(toget_sample.weights())

  # Measurement Errors (input.me and stage.me)
  eval(toget_inputstage.me())

  # Binary table for offsets
  eval(toget_offset.tables())

  # Dependent variable
  eval(toget_response.frame())

  # Apply na.action
  eval(toget_na.action.done())

  #* Create a vector of default 'names' of the vector of model parameters to be estimated
  eval(toget_all_par_bnames())

  #* Create index for slopes/intercepts
  eval(toget_par_indices())

  #* objects created and used but not returned:
  # stage.covs.name
  # stage.terms.labels
  # all.terms.labels
  # all.terms.orders
  # input.matrix
  # stage.contrasts.labels
  # all.contrasts.labels

  ########################### Format Outputs ############################
  out <- structure(list(y = if (attr(attr(mainframe, "terms"), "response")) y,
                        weights = ntrials,
                        sample.weights = sample.weights,
                        intercepts = intercepts,
                        input.matrix = design.matrix,
                        offset.matrix = offset.matrix,
                        # Measurement error inputs
                        me = includes.me,
                        sd.input.matrix = contrasts.sd.matrix,
                        me.offset = includes.me.offset,
                        sd.offset.matrix = sd.offset.matrix,
                        # Dictionaries for model components
                        stage.dictionary = stage.dictionary,
                        alpha.dictionary = alpha.dictionary,
                        lambda.dictionary = lambda.dictionary,
                        kappa.dictionary = kappa.dictionary,
                        #
                        stage.terms = list(names = stage.terms.labels,
                                           order = stage.term.order),
                        auxy = list(yname = yname, ycolname = ycolname,
                                    terms.orders = all.terms.orders,
                                    covs_labels = all_covs.name,
                                    num.covs_labels = num_covs.name,
                                    terms.me_list = terms.me_list),
                        dims = list (nobs = nobs,
                                     q = q,   # Number of stages in the MSB model
                                     npars = p + d + r + s, # Total number of model parameters in the MSB model
                                     p = p,   # Total number of intercepts and contrasts in all stages: sum(pj) + sum(intercepts)
                                     pi = sum(intercepts), # Total number of intercepts in all stages: sum(intercepts)
                                     pj = pj, # Vector of number of contrasts per stage
                                     d = d,   # Total number of intercepts and contrasts in 'alpha'
                                     r = r,   # Total number of intercepts and contrasts in 'lambda'
                                     s = s,   # Total number of intercepts and contrasts in 'kappa'
                                     pt = pt, # Total number of terms in all stages: sum(tj)
                                     tj = tj, # Vector of number of terms per stage
                                     nb_contrasts = n_contrasts,
                                     nb_covs = n_covs,
                                     nb_me.cols = n_me.cols),
                        parnames = parnames,
                        parindex = parindex,
                        tables = list(stage.contrasts = stage.contrasts_table,
                                      stage.terms = stage.terms_table,
                                      stage.offsets = stage.offsets_table,
                                      stage.covs = stage.covs_table,
                                      term.contrasts = term.contrasts_table,
                                      covs.terms = covs.terms_table,
                                      me.contrasts = me.contrasts_table),
                        frames = if (frames) list(main = mainframe,
                                                  stages = stage.frame,
                                                  alpha = alpha.frame,
                                                  lambda = lambda.frame,
                                                  kappa = kappa.frame,
                                                  input.me = me.frame,
                                                  stage.me = stage.me.frame), # me.model = NULL
                        call = mcall),
                   class = c('msbm.frame'),
                   response = attr(attr(mainframe, "terms"), "response"),
                   na.action = applied.na.action)

  return(out)
}

#* Have a function that take the an index (and an
# argument filter = subset) and return the design matrix

# Register the "msbm.frame" class
# setOldClass("msbm.frame")

# Define an S4 class 'msbm.frame'
#setClass("msbm.frame",
#         slots = c(y = "vector",
#                   weights = "vector",
...
is.msbm.frame <- function(x) {
  if (inherits(x, what = 'msbm.frame')) {
    return(TRUE)
  }
  else {
    stop("Routines to check 'msbm.frame' class not yet implemented ")
  }
}
