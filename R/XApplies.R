...
# Note that the argument 'cl' comes after '...', and must thus be explicitly named in a call.
# (same for 'chunk.size')
...
# system.time(sum(msbreg::matteSapply (x, FUN=sum))) 48.480  292.824 1397.304
# system.time(sum(msbreg::matteSapply (x, FUN=sum, cl = cl))) 43.485 226.251 895.329
...
# system.time(sum(msbreg::matteSapply (x, FUN=sum))) 0.288   0.005   0.293
# system.time(sum(msbreg::matteSapply (x, FUN=sum, cl = cl))) 0.218   0.048   0.270
