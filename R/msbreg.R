#' Fitting Multistage Binomial Regression Models
#'
...
#'
# As the main user-level function of the \code{msbreg} package, \code{msbreg}
# wraps the functions \link[msbreg]{msbm.frame} and \link[msbreg]{msbm.fit}.
# The first converts inputs from the user interface provided by
# \code{msbreg} to the design matrices needed for the subsequent model
# fitting.
#'
#' @param formula an object of class "\link[stats]{formula}" (or one
...
#'
#' @param input.me,stage.me optional list of one sided \link[stats]{formula}
...
#' for details.
#'
#' @param alpha.formula,lambda.formula optional one sided \link{formula} objects
...
# For \code{msbm.fit}, only an object of class \code{"link"} is accepted.
#'
...
#' available.
#'
#' **PS**: for \code{msbreg}, when specifying \code{link} as a function call
...
#'
#' @param data,weights,subset,na.action Same as for \link[stats]{glm}.
#' See \link[msbreg]{msbm.frame} for details.
#'
#' @param offset place holder, not used. It appears as a named argument
#' only to ensure that it is not included in the \code{...} arguments.
#' See section **Details** in \link[msbreg]{msbm.frame} for how to
#' include offsets in MSB model components.
#'
#' @param sample.weights \code{NULL} (default) or an optional vector of subject
#' weights in the sampled population. See section *Observation weights* in
#' \link[msbreg]{msbm.frame} for details.
#'
#' @param start \code{NULL} (default) or a vector of starting values for model
...
#'
#' @param control a list of parameters passed to \link[msbreg]{msbm.control}
...
#'
#' @param y For \code{msbreg}: logical value indicating whether the
...
#'
#' @param frame For \code{msbreg}: a logical value indicating whether the
#' *model frame* (of class "\link[msbreg]{msbm.frame}") should be included
...
#'
# Next versions will have the alternative \code{"gnm.fit"} which uses
# iterative re-weighted least squares (IWLS).
#'
#' @param singular.ok logical, if \code{FALSE} a singular fit is an error.
#' A *singular fit* is defined as one with a singular Fisher information matrix.
#'
#' @param ... further named arguments to pass to or from other methods.
...
#' See section **Details** in \link[msbreg]{msbreg-package} for a brief
...
#' See the section **Details** in \link[msbreg]{msbm.frame}
...
#' vector of *sample importances* is scaled to sum up to the number of
...
#' *(i)* \code{"MLJ"} (the default) for Penalized \code{"ML"} with Jeffreys
#' invariant prior as penalty, *(ii)* \code{"MLJIC"} for \code{"MLJ"} followed
...
#' are unbiased to the first order, and *(iii)* \code{"MLPJ"} for partial
...
#' (\code{-1} or \code{1}).}
#'
#' \item{\code{unit.lambda}}{logical indicating whether a *unit asymptote*
...
#'
#' @return \code{msbreg} returns an object inheriting from class
...
#' and \eqn{k = 2} for \code{aic} (*Akaike's Information Criterion*),
#' \eqn{k = log(n)} for \code{bic} (*Schwarz's Bayesian Criterion*), and
#' \eqn{k = 2 log(log(n))} for \code{hqc} (*Hannan–Quinn Information Criterion*);}
#'
#' \item{\code{deviance}}{ the *residual deviance*, that is, minus twice
...
#'  *weights* (cases) and *sample weights* (sampling weights);}
#'
...
#'
#' @author Chenangnon Tovissode working under Craig Miller at the
...
# # See '?bootfit.msbm' for a deeper exploration.
#'
...
#mdf$x1err <- mdf$x1 + rbridge(n = 1000, scale = 1/(3*mdf$SDx1 / (pi^2) + 1))
#mdf$x2err <- mdf$x2 + rbridge(n = 1000, scale = 1/(3*mdf$SDx2 / (pi^2) + 1))
#'
...
msbreg <- function (formula,
  ...
  if (is.null(mf$y)) {
    stop("cannot fit a model without a response" )
  }

  #* Control argument
  eval(get.msbm.control())

  if (identical(method, "msbm.fit") & identical(control$criterion, "ML")) {
    #* Check identifiability
    #stopifnot (identifiability(mf))
  }
  ...
  structure(c(fit,
              list(call = mcall, formula = formula,
                   link = link, dispersion = 1, data = data,
                   control = control, method = method,
                   dims = mf$dims,
                   ylab = c(yname = mf$auxy$yname,
                            ycolname = mf$auxy$ycolname),
                   frame = if (frame) mf)),
            class = c(fit$class, c("msbm")))
}

# write 'msbmer' for mixed effect models, adding argument cluster (e.g. cluster = ~ id)

# Register the "msbm" class
# setOldClass("msbm")

get.msbm.control <- function () {
  expression({
    if (!exists("control")) {
      control <- msbm.control()
    }
    else if (!length(names(control))) {
      # Because control = list(...) by default, any ... input would already be in control which would not be empty.
      control <- msbm.control()
    }
    else {
      ok.control <- names(msbm.control())
      ...
      if (any(class(dotnames) %in% c("simpleError", "error",
                                          "condition", "try-error"))) {
        alldots <- list(...)
        dotnames <- names(alldots)
        dotok <- FALSE
      }

      if (!missing(control)) {
        control <- control[names(control) %in% ok.control]

        add.control <- dotnames %in% ok.control
        if (length(names(control)))
          add.control <- add.control & !(dotnames %in% names(control))

        if (any(add.control)) {
          add.control.nm <- dotnames[which(add.control)]
          if (dotok)
            add.control.val <- ...elt(which(add.control))
          else
            add.control.val <- alldots[which(add.control)]

          nmc0 <- names(control)
          control <- c(add.control.val, control)
          names(control) <- c(add.control.nm, nmc0)
        }
      }
      else {
        control <- control[dotnames %in% ok.control]
      }
...
get_build_mf <- function() {
  expression({
      #* Build the call to msbm.frame
      if (missing(data))
        data <- environment(formula)

      mf <- match.call(expand.dots = FALSE)
      m <- match(x = c("formula", "input.me", "stage.me",
                       "alpha.formula", "lambda.formula", "kappa.formula",
      ...
      #* Build the "msbm.frame",
      # taking data from 'data', parent.frame(), or environment(formula)
      # whichever works
      mf0 <- mf
      mf <- catch.conditions({
        eval(mf0, parent.frame())
      })$value

      if (any(class(mf) %in% c("simpleError", "error",
                               "condition", "try-error"))) {
        mf1 <- mf0
        mf1$data <- data
        mf <- catch.conditions({
          eval(mf1, parent.frame())
        })$value
      ...
      #* Zero any NAs in measurement errors
      if (mf$me) {
        NA_in_ME <- is.na(mf$sd.input.matrix)
        if (any(NA_in_ME))
          mf$sd.input.matrix[NA_in_MENA_in_ME] <- 0
      }
      if (mf$me.offset) {
        NA_in_ME <- is.na(mf$sd.offset.matrix)
        if (any(NA_in_ME))
          mf$sd.offset.matrix[NA_in_ME] <- 0
      }
      mf$call <- mfcall
    })
}
